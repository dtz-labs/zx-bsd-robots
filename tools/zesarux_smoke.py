...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Smoke-test the Robots ZX 48K TAP in headless ZEsarUX."
    )
...
def send_ascii(connection: socket.socket, text: str) -> None:
    if not text:
        return
    codes = " ".join(str(ord(character)) for character in text)
    command(connection, f"send-keys-ascii {KEY_DELAY_MS} {codes}")
    time.sleep(0.2 + len(text) * KEY_DELAY_MS / 1000.0)
